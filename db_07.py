    ...
    while(True):
        for k in range(len(rows)):
            if rows[k][0] == i:
                ...
                break
        if checked != 0:
            messagebox.showerror('회원가입 싪패', '입력하신 {0}는 사용중인 아이디입니다.'.format(i))
            break
        else:
            cur.execute('insert into member (id, pw) values (?, ?)', (i, p))
            messagebox.showinfo('회원가입 성공', '회원가입이 성공적으로 이루어졌습니다.')
            break
...
btn_join = tkinter.Button(w, text='Join', command=join)
...
btn_join.grid(row=2, column=0, sticky='w')
